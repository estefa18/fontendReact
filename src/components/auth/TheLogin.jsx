...
import { useNavigate } from 'react-router-dom'; // Lo usamos para cambiar de rutas sin tener que acurdir a los links
...
const TheLogin = () => {
  ...
  const {isLoggedIn, login, logout} = useAuth();
    ...
    //Manejadores
  const [currentTab, setToggleState] = useState(1); // currentTab la variable que contiene el estado, setToggleState este depende la acción, use = es una seña enr eact es para indicar cambio de estado  
  // Variable, función (set, le entrega -> Nombre de referecnias) = estado a utilizar y se pueden iniciar de pendiendo de su funcionalidad
  ...
  const handleChange = (event) =>{
    ...
    setData({
      ...dataUser,// .. -> para desgregar un arregle o ara capturar información de un arreglo 
      [event.target.name]: event.target.value //Evento, objetivo y el nombre
      /* Esto es lo que hace [event.target.name]:
        {
            email:micorreo@
            passaword: 123
        }
    ...
    <div className="the-login">
      ...
      <div className="contenedor-formularios">
        ...
        <ul className="contenedor-tabs">
          <li
            className={currentTab === 1 ? "active" : ""}  //const [currentTab, setToggleState] = useState(1);
            onClick={() => selectTab(1)}
